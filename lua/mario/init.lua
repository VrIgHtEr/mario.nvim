local I, T, C = require("meow.image"), require("pixel.terminal"), require("mario.characters")
	...
	for _, c in ipairs(C.data) do
		function c.update(state)
			...
			elseif c.state == "animating" then
				...
				c.anim.cur_frame = math.floor(
					(c.frame_counter / c.anim.speed * c.speed)
						% (c.anim.frames > 1 and c.anim.frames - 2 + c.anim.frames or 1)
				)
